...
const CampSite = ({  backgroundImage, peopleJoined, subtitle, title } : CampSiteProps ) => {
  return (
    <div className={`h-full w-full min-w-[1100px] ${backgroundImage}  bg-no-repeat lg:rounded-tr-5xl 2xl:rounded-5xl`}>
          ...
          <span className='flex -space-x-4 overflow-hidden'>
            {PEOPLE_URL.map(url => (
              <Image
                src={url}
                height={52}
                className='inline-bloc h-10 w-10 rounded-full'
                width={52}
                alt='url'
                key={url}
              />
